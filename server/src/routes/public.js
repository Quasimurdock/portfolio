...
router.get(
  ...
  asyncHandler((_req, res) => {
    const rows = all('SELECT * FROM sections WHERE visible = 1 ORDER BY position ASC, key ASC')
...
function resolveLink(row) {
  ...
  if (row.link_kind === 'collection' && row.target_collection_id) {
    const target = get('SELECT slug, section_key FROM collections WHERE id = ? AND status = ?', [
  ...
  if (row.link_kind === 'article' && row.target_article_id) {
    const target = get('SELECT slug, section_key FROM articles WHERE id = ? AND status = ?', [
...
function publicCover(row) {
  if (row.cover_image_id) {
    const explicit = get(`SELECT * FROM images WHERE id = ? AND ${PUBLISHED}`, [row.cover_image_id])
    if (explicit) return explicit
  }
  return (
    get(`SELECT * FROM images WHERE collection_id = ? AND ${PUBLISHED} ORDER BY position ASC, id ASC LIMIT 1`, [
      row.id,
    ]) ?? null
  )
}
...
router.get(
  ...
  asyncHandler((req, res) => {
    const section = get('SELECT * FROM sections WHERE key = ? AND visible = 1', [req.params.key])
    ...
    if (section.kind === 'grid') {
      ...
      payload.collections = rows.map((row) =>
        toCollection(row, { forPublic: true, imageCount: row.image_count, cover: publicCover(row) }),
      )
...
router.get(
  '/collections/:slug',
  asyncHandler((req, res) => {
    const row = get(`SELECT * FROM collections WHERE slug = ? AND ${PUBLISHED}`, [req.params.slug])
    ...
    const cover = publicCover(row) ?? images[0] ?? null
...
router.get(
  '/articles/:slug',
  asyncHandler((req, res) => {
    const row = get(`SELECT * FROM articles WHERE slug = ? AND ${PUBLISHED}`, [req.params.slug])
    if (!row) throw notFound('No such article')

    const cover = row.cover_image_id
      ? get(`SELECT * FROM images WHERE id = ? AND ${PUBLISHED}`, [row.cover_image_id])
      : null
...
router.get(
  '/pages/:slug',
  asyncHandler((req, res) => {
    const row = get(`SELECT * FROM pages WHERE slug = ? AND ${PUBLISHED}`, [req.params.slug])
